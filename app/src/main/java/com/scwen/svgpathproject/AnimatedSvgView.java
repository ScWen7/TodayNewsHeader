...
//  private long mStartTime;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            mViewportWidth = a.getInt(R.styleable.AnimatedSvgView_animatedSvgImageSizeX, 200);
            aspectRatioWidth = a.getInt(R.styleable.AnimatedSvgView_animatedSvgImageSizeX, 200);
            mViewportHeight = a.getInt(R.styleable.AnimatedSvgView_animatedSvgImageSizeY, 200);
            aspectRatioHeight = a.getInt(R.styleable.AnimatedSvgView_animatedSvgImageSizeY, 200);
...
//          (t - (mTraceTime - mTraceTimePerGlyph) * i * 1f / mGlyphData.length) * 1f / mTraceTimePerGlyph);
                ...
                float distance = phase * mGlyphData[i].length;
...
//            float phase = constrain(0, 1, (t - mFillStart) * 1f / mFillTime);
            float v = constrain(0, 0.2f, phase - 1f);

            for (int i = 0; i < mGlyphData.length; i++) {
                GlyphData glyphData = mGlyphData[i];
                int fillColor = mFillColors[i];
                int a = (int) (v * 5.0f * ((float) Color.alpha(fillColor) / (float) 255) * 255);
...
//    ViewCompat.postInvalidateOnAnimation(this);
    ...
    public void startBlink() {
        ...
        if (mValueAnimator == null) {
            ...
            mValueAnimator = ValueAnimator.ofFloat(1.2f, 1.0f, 1.2f);
